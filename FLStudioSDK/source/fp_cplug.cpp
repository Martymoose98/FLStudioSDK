...
#include <stdio.h>
#include "math.h"
#include "../include/fp_cplug.h"
#include "../include/fp_extra.h"
#include <string.h>
...
{
    char tempc[256];
    sprintf_s(tempc, "%d%%", Value*100 / Max);
...
{
    const char* PitchTypeT[4] = { "octave", "semitone", "cent", "Hz" };

    char Msg[256];
    char M2[256];
    sprintf_s(M2, "%d %s", Value, PitchTypeT[PitchType+1]);   // PitchType = -1..2
...
void TCPPFruityPlug::ShowHintMsg_Pitch(float Value, int PitchType, int Digits)
{
    ...
    sprintf_s(M2, "%%%d.f %%s", Digits);  // create the format string
    sprintf_s(M2, M2, Value, PitchTypeT[PitchType+1]);
...
void TCPPFruityPlug::ShowHintMsg_Pan(int Value)
{
    ...
    n = Value * 100 / 64;
    if (n == 0)
        strcpy_s(Msg, "Centered");
    else
    {
        sprintf_s(Msg, "%d%% ", abs(n));

        if (n < 0)
        {
            strcat_s(Msg, "left");
            //strcpy_s(Msg, strcat_s(Msg, "left"));
        }
        else
        {
            strcat_s(Msg, "right");
            //strcpy_s(Msg, strcat_s(Msg, "right"));
        }
    }
...
void TCPPFruityPlug::ShowHintMsg_Time(int Value)
{
    ...
    sprintf_s(Msg, "%d:%.2d", Value / AbsPPN, Value % AbsPPN);
    PlugHost->OnHint(HostTag, Msg);
}
